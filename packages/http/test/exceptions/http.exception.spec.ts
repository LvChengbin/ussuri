...
describe( 'http-exception', () => {
    ...
    it( 'set error description directly', () => {
        const exception = new HttpException( 400, 'something error' );
        expect( exception.message ).toEqual( 'something error' );
    } );

    it( 'set error description in response object', () => {
        const exception = new HttpException( { status : 400, statusText : 'something error' } );
        expect( exception.message ).toEqual( 'something error' );
    } );
