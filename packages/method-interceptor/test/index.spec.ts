...
describe( 'method-interceptor', () => {
    ...
    describe( 'full features', () => {
        ...
        const Purification = createDecoratorAfter( async ( metadata: MetadataAfter, value: UserInfo, ctx: Context ): Promise<Omit<UserInfo, 'password'>> => {
            if( ctx.query.forceError ) {
                throw new CustomError( 'Custom error' );
            }
            delete value.password;
            return value as Omit<UserInfo, 'password'>;
        } );
